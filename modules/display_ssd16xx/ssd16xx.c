/*
 * Vendored copy of Zephyr's SSD16XX display driver
 * (zephyr/drivers/display/ssd16xx.c) with a fix for gen2 (SSD168x)
 * partial refresh buffer maintenance. Kept out-of-tree so eno-os is
 * self-contained; built via ZEPHYR_EXTRA_MODULES with CONFIG_SSD16XX=n.
 *
			 ...
			 const void *buf)
{
	...
	if (data->blanking_on && have_partial_refresh) {
		...
		/*
		 * We just performed a partial refresh. The gen2
		 * controllers (SSD168x) do not reliably keep the
		 * previous-image RAM in sync after a MODE2 refresh,
		 * and the exact buffer switch behavior is
		 * undocumented. The only safe way to keep
		 * differential updates working is to write the
		 * current image into both RAM buffers after the
		 * refresh (same approach as GxEPD2).
		 */
			       ...
			       enum ssd16xx_profile_type type)
{
	...
	if (p && p->lut.len &&
	    (quirks->ctrl2_partial & SSD16XX_GEN2_CTRL2_MODE2)) {
		/*
		 * Gen2 waveform settings include an "Option for LUT
		 * end" byte (register 0x3f) right after the LUT. The
		 * vendor partial waveform for SSD168x panels uses
		 * 0x02 for it; without it the register waveform is
		 * applied incompletely.
		 */
		err = ssd16xx_write_uint8(dev, SSD16XX_CMD_END_OPTION,
					  SSD16XX_GEN2_END_OPTION_PARTIAL);
		if (err < 0) {
			return err;
		}
	}
